...
local function isVehicleOwned(plate)
    if Config.breezyFakePlate then
        local hasFakePlate = exports['breezy-fakeplates']:getPlateFromFakePlate(plate)
...
RegisterNetEvent('breezy-harness:server:attachHarness', function(plate, ItemData)
    local src = source
    if not src then return end
    if not isVehicleOwned(plate) then return TriggerClientEvent('seatbelt:client:UseHarness', src, ItemData, true) end
    if Config.UninstallHarnessWithItem and hasHarness(plate) then return TriggerClientEvent('breezy-harness:client:installHarness', src, plate, 'uninstall') end

    TriggerClientEvent('breezy-harness:client:installHarness', src, plate, 'install')
end)

RegisterNetEvent('breezy-harness:server:installHarness', function(plate, action)
    local src = source
...
RegisterNetEvent('breezy-harness:server:uninstallHarness', function(plate)
    local src = source
    if not plate then return end
...
RegisterNetEvent('breezy-harness:server:toggleBelt', function(plate, ItemData)
    local src = source
    if not src then return end
